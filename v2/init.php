...
function check_login() {
	...
	if($_REQUEST['action'] == 'login'){
		if(empty($_REQUEST['username'])){
			...
			$setCookie	= true;
			$ipbwi->member->login($_REQUEST['username'],$_REQUEST['password'],$setCookie);
			#$ipbwi->boink_it($url=$_SERVER['PHP_SELF'], $msg="You are being logged in...");
	...
	if($_REQUEST['action'] == 'logout'){
		$ipbwi->member->logout();
		//header('location: '.$_SERVER['PHP_SELF']);
		$ipbwi->boink_it($_SERVER['PHP_SELF'], $msg="You are being logged out...");
	}
}
// Loads stats
...
require(ROOT.'ipbwi/lib/ipsearch.inc.php');
